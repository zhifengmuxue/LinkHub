<template>
    <div class="tool-box">
      ...
      <a-card class="tool-card">
        ...
        <a-button type="primary" @click="shortenUrl" :loading="isShortening" block>
          生成短链接
        </a-button>
      ...
      <a-card class="tool-card">
        ...
        <a-button type="primary" @click="generateQrCode" :loading="isGeneratingQr" block>
          生成二维码
        </a-button>
  ...
  const shortenUrl = async () => {
    ...
    try {
      shortUrl.value = `https://short.url/${Math.random().toString(36).slice(2, 8)}`;
